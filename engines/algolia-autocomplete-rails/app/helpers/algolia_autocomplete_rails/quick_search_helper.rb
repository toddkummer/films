...
module AlgoliaAutocompleteRails
  ...
  # - sources: A hash with the search key as the key and the name of the source as the value. If there is only one
  #     source and no search key is required, the source can be passed as a string.
  ...
  module QuickSearchHelper
    def quick_search(sources, placeholder: 'Search', **html_options)
      ...
      data = (html_options.delete(:data) || {})
             .merge(controller: 'algolia-autocomplete-rails--quick-search',
                    algolia_autocomplete_rails__quick_search_placeholder_value: placeholder,
                    algolia_autocomplete_rails__quick_search_source_mapping_value: sources.to_json)
      tag.div(data: data, **html_options) do
        tag.div(data: { algolia_autocomplete_rails__quick_search_target: 'searchInput' })
      end
    end
  end
end
